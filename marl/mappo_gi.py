...
from marl.single_agent.Model_gi import ActorNetwork, CriticNetwork, ActorCriticNetwork
...
class MAPPO_GI:
    ...
    def __init__(
        ...
        assert (
            th.cuda.is_available() if use_cuda else True
        ), "GPU is not available! Please enable GPU access to train."
        ...
        if not self.shared_network:
            ...
            if self.use_cuda:
                if th.cuda.is_available():
                    ...
                    print(
                        "Could not determine the GPU with the most available memory. Using CPU"
                    )
                    self.use_cuda = False
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            if not self.shared_network:
                ...
                old_action_log_probs = self.actor_target(
                    states_var[:, agent_id, :]
                ).detach()
                old_action_log_probs = th.sum(
                    old_action_log_probs * actions_var[:, agent_id, :], 1
                )
                ...
                if (
                    self.n_episodes % self.target_update_steps == 0
                    and self.n_episodes > 0
                ):
                ...
                action_log_probs = self.policy(states_var[:, agent_id, :])
                action_log_probs = th.sum(
                    action_log_probs * actions_var[:, agent_id, :], 1
                )
                ...
                # actor loss
                old_action_log_probs = self.policy_target(
                    states_var[:, agent_id, :]
                ).detach()
                old_action_log_probs = th.sum(
                    old_action_log_probs * actions_var[:, agent_id, :], 1
                ...
                if (
                    ...
                    self._soft_update_target(self.policy_target, self.policy)
    ...
    def _softmax_action(self, state, n_agents):
        ...
        for agent_id in range(n_agents):
            if not self.shared_network:
                ...
                softmax_action_var = th.exp(self.policy(state_var[:, agent_id, :]))
    ...
    def value(self, state, action):
        ...
        for agent_id in range(self.n_agents):
            if not self.shared_network:
                value_var = self.critic(state_var[:, agent_id, :])
            else:
    ...
    def evaluation(self, env, output_dir, eval_episodes=1, is_train=True):
        ...
        for i in range(eval_episodes):
            ...
            if is_train:
                if self.traffic_density == 1:
                    state, action_mask = env.reset(
                        is_training=False, testing_seeds=seeds[i], num_CAV=(i + 1) % 3
                    )
                elif self.traffic_density == 2:
                    state, action_mask = env.reset(
                        is_training=False, testing_seeds=seeds[i], num_CAV=(i + 2) % 4
                    )
                elif self.traffic_density == 3:
                    state, action_mask = env.reset(
                        is_training=False, testing_seeds=seeds[i], num_CAV=(i + 4) % 6
                    )
            else:
                state, action_mask = env.reset(
                    is_training=False, testing_seeds=seeds[i]
                )
            ...
            if output_dir is not None:
                ...
                video_filename = os.path.join(
                    output_dir,
                    "testing_episode{}".format(self.n_episodes + 1)
                    + "_{}".format(i)
                    + ".mp4",
                )
            ...
            if video_filename is not None:
                print(
                    "Recording video to {} ({}x{}x{}@{}fps)".format(
                        video_filename, *rendered_frame.shape, 5
                    )
                )
                video_recorder = VideoRecorder(
                    video_filename, frame_size=rendered_frame.shape, fps=5
                )
            ...
            while not done:
                ...
                if "traffic_speed" in info:
                    traffic_speed = (
                        traffic_speed + info["traffic_speed"]
        ...
        if min_headway < env.config["HEADWAY_TIME"]:
            print(
                "Minimum headway violation in seed {2}: found {0}, but expected >{1}".format(
                    min_headway, env.config["HEADWAY_TIME"], seeds[i]
                )
            )
...
class MAPPOControlEval(MAPPO_GI):
    def evaluation(self, env: "MergeEnvLCMARL", output_dir):
        rewards, vehicle_info, ext_info = super().evaluation(
            env=env, output_dir=output_dir, eval_episodes=1, is_train=False
        )
